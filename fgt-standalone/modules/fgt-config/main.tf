...
data "template_file" "fgt_bgp-config" {
  ...
  vars = {
    ...
    router_id = var.fgt_ni_ips["public"]
  }
}

locals {
  fgt_fgsp_vxlan_config =  compact([for i, peer_vxlan_config in data.template_file.fgt_vxlan-active-config.*.rendered  : i % 2 != 0 ? null : peer_vxlan_config])
}
